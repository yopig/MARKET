  ...
  const pickThumb = (files) => {
    ...
    // 문자열/객체 혼재 대응
    const urlOf = (f) =>
      typeof f === "string" ? f : f?.url || f?.path || f?.imageUrl || f?.thumbnailUrl || f?.name || "";
                  ...
                  <div className="file-list">
                    {board.files.map((f, i) => {
                      const name =
                        typeof f === "string"
                          ? f.split("/").pop()
                          : f?.originalName || f?.name || f?.url || `file-${i+1}`;
                      const url =
                        typeof f === "string" ? f : f?.url || f?.path || f?.imageUrl || null;
                      return (
